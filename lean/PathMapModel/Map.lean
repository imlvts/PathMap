...
A `pathmap::PathMap` *is* a `Trie`: the value at the empty path is the map's
...
/-- `PathMap::new`. -/
def new : Trie V := Trie.empty

/-- `PathMap::single`. -/
def single (p : Path) (v : V) : Trie V := (Trie.empty.setVal p v).2

/-- A read/write zipper at the root of the map. -/
def zipper (t : Trie V) : Zip V := { trie := t, root := [], path := [] }
...
value at `p`. -/
def zipperAt (t : Trie V) (p : Path) : Zip V := { trie := t, root := p, path := [] }
...
def setValAt (t : Trie V) (p : Path) (v : V) : Option V × Trie V := t.setVal p v

/-- `PathMap::remove_val_at` / `remove` (`remove` passes `prune = true`). -/
def removeValAt (t : Trie V) (p : Path) (prune : Bool) : Option V × Trie V :=
  let (old, z) := ((zipper t).descendTo p).removeVal prune
  (old, z.trie)

/-- `PathMap::val_count`. -/
def valCount (t : Trie V) : Nat := t.valCount []

/-- `PathMap::is_empty`. -/
def isEmpty (t : Trie V) : Bool := t.isEmptyMap

/-- `PathMap::create_path`. -/
def createPath (t : Trie V) (p : Path) : Bool × Trie V :=
  let (b, z) := ((zipper t).descendTo p).createPath
  (b, z.trie)

/-- `PathMap::prune_path`. -/
def prunePath (t : Trie V) (p : Path) : Nat × Trie V :=
  let (n, z) := ((zipper t).descendTo p).prunePath
  (n, z.trie)

/-- `PathMap::remove_branches_at`. -/
def removeBranchesAt (t : Trie V) (p : Path) (prune : Bool) : Bool × Trie V :=
  let (b, z) := ((zipper t).descendTo p).removeBranches prune
  (b, z.trie)

/-- All key/value pairs, in depth-first (lexicographic) key order — `PathMap::iter`. -/
def iter (t : Trie V) : List (Path × V) := t.vals

variable (ops : ValOps V)

/-- `PathMap::join`: union, root values included. -/
def join (a b : Trie V) : Trie V := Trie.join ops a b

/-- `PathMap::meet`: intersection, root values included. -/
def meet (a b : Trie V) : Trie V := Trie.meet ops a b

/-- `PathMap::subtract`: difference, root values included. -/
def subtract (a b : Trie V) : Trie V := Trie.sub ops a b
...
def restrict (a b : Trie V) : Trie V :=
  if (b.valAt []).isSome then a else Trie.restrictBelowRoot a b
